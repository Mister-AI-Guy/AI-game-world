  ...
  constructor(inputSize, hiddenSizes, outputSize) {
    ...
    for (let i = 0; i < sizes.length - 1; i++) {
      ...
      for (let j = 0; j < sizes[i + 1]; j++) {
        const neuron = [];
        for (let k = 0; k < sizes[i]; k++) neuron.push((Math.random() * 2 - 1));
        layer.push(neuron);
      }
  ...
  predict(inputs) {
    ...
    for (let l = 0; l < this.layers.length; l++) {
      ...
      for (let j = 0; j < this.layers[l].length; j++) {
        ...
        for (let k = 0; k < current.length; k++) sum += this.layers[l][j][k] * current[k];
  ...
  mutate(rate = 0.1, strength = 0.2) {
    for (let l = 0; l < this.layers.length; l++) {
      for (let j = 0; j < this.layers[l].length; j++) {
        ...
        if (Math.random() < rate) this.biases[l][j] += (Math.random() * 2 - 1) * strength;
      }
    }
    return this;
  }
    ...
    for (let l = 0; l < child.layers.length; l++) {
      for (let j = 0; j < child.layers[l].length; j++) {
        for (let k = 0; k < child.layers[l][j].length; k++) {
          if (Math.random() < 0.5) child.layers[l][j][k] = other.layers[l][j][k];
        }
...
class GeneticAlgorithm {
  ...
  setFitness(index, fitness) { this.fitnesses[index] = fitness; }
  ...
  evolve() {
    ...
    const newPop = [];
    for (let i = 0; i < this.eliteCount; i++) newPop.push(this.population[ranked[i].i].clone());
    while (newPop.length < this.populationSize) {
      const p1    = this._tournamentSelect(ranked);
      const p2    = this._tournamentSelect(ranked);
